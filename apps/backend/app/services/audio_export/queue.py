...
if TYPE_CHECKING:
    from app.services.audio_export.models import ExportJob, ExportJobState
...
class ExportQueue:
    def enqueue(self, job: "ExportJob") -> "ExportJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "ExportJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "ExportJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "ExportJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
