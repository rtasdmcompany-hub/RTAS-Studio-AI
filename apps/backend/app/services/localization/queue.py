"""Localization queue — translating / voice_generation / lip_sync / subtitle_generation."""
...
if TYPE_CHECKING:
    from app.services.localization.models import LocalizationJob, LocalizationJobState

_lock = threading.Lock()
_queue: deque[str] = deque()
_jobs: OrderedDict[str, "LocalizationJob"] = OrderedDict()
_enqueued_at: dict[str, float] = {}
_MAX = 2000


class LocalizationQueue:
    def enqueue(self, job: "LocalizationJob") -> "LocalizationJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "LocalizationJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "LocalizationJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "LocalizationJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "LocalizationJob | None":
        with _lock:
...
localization_queue = LocalizationQueue()
