"""Voice job queue — mirrors Audio Queue lifecycle states."""
...
if TYPE_CHECKING:
    from app.services.voice_generation.models import VoiceGenerationJob, VoiceJobState
...
class VoiceQueue:
    def enqueue(self, job: "VoiceGenerationJob") -> "VoiceGenerationJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "VoiceGenerationJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "VoiceGenerationJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "VoiceGenerationJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "VoiceGenerationJob | None":
        with _lock:
