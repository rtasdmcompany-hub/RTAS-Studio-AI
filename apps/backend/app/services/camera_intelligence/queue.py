"""Production queue for camera intelligence jobs."""
...
if TYPE_CHECKING:
    from app.services.camera_intelligence.models import CameraIntelligenceJob, CameraJobState
...
class CameraQueue:
    def enqueue(self, job: "CameraIntelligenceJob") -> "CameraIntelligenceJob":
        with _lock:
            job.state = "queued"
    ...
    def retry(self, job_id: str) -> "CameraIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "CameraIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "CameraIntelligenceJob | None":
        with _lock:
