...
if TYPE_CHECKING:
    from app.services.audio_timeline.models import TimelineJob, TimelineJobState
...
class TimelineQueue:
    def enqueue(self, job: "TimelineJob") -> "TimelineJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "TimelineJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "TimelineJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "TimelineJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "TimelineJob | None":
        with _lock:
