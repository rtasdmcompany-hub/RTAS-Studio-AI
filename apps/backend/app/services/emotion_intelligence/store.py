"""In-memory emotion intelligence job store."""
...
def save(job: "EmotionIntelligenceJob") -> "EmotionIntelligenceJob":
    with _lock:
...
def get(job_id: str) -> "EmotionIntelligenceJob | None":
    with _lock:
        return _jobs.get(job_id)


def history(limit: int = 50) -> list["EmotionIntelligenceJob"]:
    with _lock:
