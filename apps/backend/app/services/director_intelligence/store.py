"""In-memory director intelligence job store."""
...
def save(job: "DirectorIntelligenceJob") -> "DirectorIntelligenceJob":
    with _lock:
...
def get(job_id: str) -> "DirectorIntelligenceJob | None":
    with _lock:
        return _jobs.get(job_id)


def history(limit: int = 50) -> list["DirectorIntelligenceJob"]:
    with _lock:
