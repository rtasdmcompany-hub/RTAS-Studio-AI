"""Production queue for director intelligence jobs."""
...
if TYPE_CHECKING:
    from app.services.director_intelligence.models import DirectorIntelligenceJob, DirectorJobState

_lock = threading.Lock()
_queue: deque[str] = deque()
_jobs: OrderedDict[str, "DirectorIntelligenceJob"] = OrderedDict()
_enqueued_at: dict[str, float] = {}
_MAX = 2000


class DirectorQueue:
    def enqueue(self, job: "DirectorIntelligenceJob") -> "DirectorIntelligenceJob":
        with _lock:
            job.state = "queued"
    ...
    def retry(self, job_id: str) -> "DirectorIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "DirectorIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "DirectorIntelligenceJob | None":
        with _lock:
