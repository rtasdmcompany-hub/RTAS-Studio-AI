"""Music queue — Audio Queue lifecycle with composing/mixing states."""
...
if TYPE_CHECKING:
    from app.services.music_generation.models import MusicJob, MusicJobState
...
class MusicQueue:
    def enqueue(self, job: "MusicJob") -> "MusicJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "MusicJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "MusicJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "MusicJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "MusicJob | None":
        with _lock:
