"""Production queue for world intelligence jobs."""
...
if TYPE_CHECKING:
    from app.services.world_intelligence.models import WorldIntelligenceJob, WorldJobState
...
class WorldQueue:
    def enqueue(self, job: "WorldIntelligenceJob") -> "WorldIntelligenceJob":
        with _lock:
            job.state = "queued"
    ...
    def retry(self, job_id: str) -> "WorldIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "WorldIntelligenceJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "WorldIntelligenceJob | None":
        with _lock:
