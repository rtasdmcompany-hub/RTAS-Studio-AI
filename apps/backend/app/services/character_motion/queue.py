"""Production queue for character motion jobs."""
...
if TYPE_CHECKING:
    from app.services.character_motion.models import CharacterMotionJob, MotionJobState
...
class MotionQueue:
    def enqueue(self, job: "CharacterMotionJob") -> "CharacterMotionJob":
        with _lock:
            job.state = "queued"
    ...
    def dequeue(self) -> "CharacterMotionJob | None":
        with _lock:
            while _queue:
                jid = _queue.popleft()
                job = _jobs.get(jid)
    ...
    def retry(self, job_id: str) -> "CharacterMotionJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "CharacterMotionJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "CharacterMotionJob | None":
        with _lock:
