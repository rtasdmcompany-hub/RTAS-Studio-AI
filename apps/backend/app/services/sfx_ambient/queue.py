"""SFX/Ambient queue — searching / generating / layering states."""
...
if TYPE_CHECKING:
    from app.services.sfx_ambient.models import SfxJob, SfxJobState
...
class SfxQueue:
    def enqueue(self, job: "SfxJob") -> "SfxJob":
        with _lock:
            job.state = "queued"
    ...
    def retry(self, job_id: str) -> "SfxJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def cancel(self, job_id: str) -> "SfxJob | None":
        with _lock:
            job = _jobs.get(job_id)
            if not job:
                return None
    ...
    def get(self, job_id: str) -> "SfxJob | None":
        with _lock:
